...
当我们与往常一样，运行它node server.js时，它会马上在命令行上输出“Server has started.”。当我们向服务器发出请求（在浏览器访问[http://localhost:8888/](http://localhost:8888/)  ），“Request received.”这条消息就会在命令行中出现。
...
有了这些，我们就把服务器、路由和请求处理程序在一起了。现在我们启动应用程序并在浏览器中访问[http://localhost:8888/](http://localhost:8888/)start，以下日志可以说明系统调用了正确的请求处理程序：
...
```
并且在浏览器中打开[http://localhost:8888/](http://localhost:8888/)可以看到这个请求同样被start请求处理程序处理了：
...
（如果想要了解更多关于事件轮询细节，可以阅读Mixu的博文——[理解node.js](http://blog.mixu.net/2011/02/01/understanding-the-node-js-event-loop/)的事件轮询。）
...
exports.upload = upload;
```
这样一来，当请求[http://localhost:8888/](http://localhost:8888/)start的时候，会花10秒钟的时间才载入，而当请求[http://localhost:8888/upload](http://localhost:8888/upload)的时候，会立即响应，纵然这个时候/start响应还在处理中。
...
*注释：
windows下面fs.js解析照片路径会报错,Error: ENOENT
...
* 本文来源[https://www.nodebeginner.org/index-zh-cn.html#about](https://www.nodebeginner.org/index-zh-cn.html#about)